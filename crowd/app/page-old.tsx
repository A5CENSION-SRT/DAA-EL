...
export default function H3CrowdControlSystem() {
  ...
  const [customLng, setCustomLng] = useState<string>('77.6076');
  const [customLat, setCustomLat] = useState<string>('12.9752');
  const [customZoom, setCustomZoom] = useState<string>('15');
  ...
  const loadCustomVenue = useCallback(async () => {
    ...
    try {
      const delta = 0.01; // ~1km search radius
      const customVenue = {
        id: 'custom',
        name: `Custom (${lat.toFixed(4)}, ${lng.toFixed(4)})`,
        description: `Custom location at ${lat.toFixed(4)}°, ${lng.toFixed(4)}°`,
        center: [lng, lat] as [number, number],
        zoom: parseFloat(customZoom) || 15,
        bbox: [lat - delta, lng - delta, lat + delta, lng + delta] as [number, number, number, number],
      };
      setGraph(await fetchOsmGraph(customVenue));
      setShowCustomVenue(false);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to fetch map data for this location');
    } finally {
      setLoading(false);
    }
  }, [customLng, customLat, customZoom]);

  useEffect(() => {
    if (venueKey !== 'custom') {
      loadVenue(venueKey);
    }
  }, [venueKey]); // eslint-disable-line
        ...
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_360px] flex-1 min-h-0">
          ...
          <aside className="flex flex-col gap-3 h-full min-h-0 overflow-y-auto pr-1">
                  ...
                  {!showCustomVenue ? (
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-800/60 px-3 py-2 text-xs font-medium text-slate-400 hover:text-blue-300 hover:border-blue-600/40 transition-colors">
                        <MapPin className="h-3 w-3" /> Custom Location
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3 rounded-lg border border-slate-700/60 bg-slate-800/40 p-3">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          placeholder="12.9752"
                          className="h-8 w-full rounded-lg border border-slate-700 bg-slate-900 px-2.5 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-blue-500/60 focus:ring-1 focus:ring-blue-500/40"
                        />
                      </div>
                      <div className="space-y-2">
                      ...
                      {error && <p className="text-xs text-red-400 bg-red-950/50 rounded px-2 py-1">{error}</p>}
                      <div className="flex gap-2">
                        <button
                          onClick={loadCustomVenue}
                          disabled={loading}
                          className="flex-1 flex items-center justify-center gap-2 rounded-lg border border-blue-600/40 bg-blue-600/20 px-3 py-2 text-xs font-medium text-blue-300 hover:bg-blue-600/30 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                        ...
                        <button
                          ...
                          disabled={loading}
                          className="flex-1 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-xs font-medium text-slate-400 hover:text-slate-200 hover:border-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                          Cancel
                        </button>
                      </div>
                    </div>
            ...
            <ZoneControls
              ...
              directionPreference={snapshot?.directionPreference ?? { enabled: false, directions: [], weight: 0 }}
              onAddZone={addZone}
              onUpdateZone={updateZone}
              onRemoveZone={removeZone}
              onSetDirectionPreference={setDirectionPreference}
              venueCenter={venue?.center}
              venueBbox={venue?.bbox}
            />
