...
import {
    Network, Activity, Timer, Hexagon, AlertTriangle, Users, Zap,
    Plus, ArrowRightFromLine, Ban, MapPin, Trash2, Settings, Gauge,
} from 'lucide-react';
...
import { CRITICAL_THRESHOLD } from '@/lib/h3-spatial';
...
export function ControlsPanel(props: ControlsPanelProps) {
    ...
    const meta = ALGORITHM_META[props.algorithm];
    const sc = speedColor(props.speed);

    return (
        <div className="overflow-y-auto pr-1 space-y-2">
            <Accordion type="single" collapsible defaultValue="sim">
                {/* ─── Simulation Settings ─── */}
                ...
                {/* ─── Dispersal Zones ─── */}
                <AccordionItem value="dispersal">
                    <AccordionTrigger className="px-2">
                        <div className="flex items-center gap-2">
                            <Gauge className="h-4 w-4 text-purple-400" />
                            <span className="text-xs font-semibold">Dispersal</span>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="px-2">
                        <ZoneControls
                            zones={props.snapshot?.zones ?? []}
                            directionPreference={props.snapshot?.directionPreference ?? { enabled: false, directions: [], weight: 0 }}
                            onAddZone={props.addZone}
                            onUpdateZone={props.updateZone}
                            onRemoveZone={props.removeZone}
                            onSetDirectionPreference={props.setDirectionPreference}
                            venueCenter={props.venueCenter}
                            venueBbox={props.venueBbox}
                        />
                    </AccordionContent>
                </AccordionItem>
                ...
                <AccordionItem value="analysis">
                    ...
                    <AccordionContent className="px-2 space-y-2">
                        {!m || m.nodesExplored === 0 ? (
                            <p className="text-xs text-slate-600 italic">Run simulation to see analysis</p>
                        ) : (
                            <div className="space-y-2">
                                {[
                                    ['Algorithm', meta?.label ?? '—'],
                                    ['Nodes explored', String(m.nodesExplored)],
                                    ['Edges relaxed', String(m.edgesRelaxed)],
                                    ['Path length', m.totalPathMetres && m.totalPathMetres < 1e8 ? `${Math.round(m.totalPathMetres)} m` : '—'],
                                ].map(([label, value]) => (
                                    <div key={label} className="flex items-baseline justify-between gap-2">
                                        <p className="text-[10px] uppercase tracking-widest text-slate-600">{label}</p>
                                        <p className="text-[11px] font-mono font-semibold text-slate-300">{value}</p>
                                    </div>
                                ))}
                            </div>
                        )}
