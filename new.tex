...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/ui_dashboard_overview.png}
...
\item Auto-detect exit nodes as the two nodes closest to opposite bounding box corners, positioned at maximum spatial separation to represent exits.
\end{enumerate}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/simulation_agent_movement.png}
\caption{Simulation agent movement and road network topology. The visualization shows agent particles (yellow dots) moving along parsed street segments at MG Road, Bangalore. Segment colors indicate traffic congestion levels, enabling operators to analyze network load dynamically. The interface includes real-time telemetry panels to track performance and evacuation metrics.}
\label{fig:network}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/algorithm_exploration_efficiency.png}
\caption{Routing algorithm exploration efficiency. The scatter plot compares physical path distance against the number of expanded graph nodes in the visited set for Dijkstra, A*, and BFS. Heuristic pruning in A* Search (slope 0.35) yields a flatter slope compared to Dijkstra (slope 0.64) and BFS (slope 2.15), demonstrating significantly better scaling as distance to destination increases.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/h3_density_visualization.png}
\caption{H3 hexagonal spatial density visualization. Agent counts are aggregated in real-time within equal-area hexagonal cells (Resolution 10, $\approx 66$ m edge length). Normalized density is color-coded from blue (low) to red (critical), identifying high-occupancy hotspots and bottlenecks. Throttling updates to every second frame preserves 60 FPS performance without loss of spatial tracking fidelity.}
\label{fig:h3density}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/population_vs_latency.png}
\caption{Evacuation scaling telemetry. The figure plots average network congestion ratio and algorithm execution runtimes as the active agent population scales from 100 to 2,000. Dijkstra and A* runtimes scale sub-linearly under load, with A* query latency remaining below 2.5 ms at peak populations, confirming the feasibility of real-time client-side evacuation simulations.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{images/rerouting_analysis.png}
\caption{Congestion-aware routing and blockage handling validation. When key arterial edges are blocked, the pathfinding solver dynamically recalculates route choices, redirecting the agent stream along alternate secondary pathways (increasing mean path length by 52\%). This confirms correct propagation of dynamic topology constraints.}
\label{fig:rerouting}
\end{figure}
